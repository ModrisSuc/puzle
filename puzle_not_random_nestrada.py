...
for piece in pieces:
    ...
    for i in range(4):                                 # enables the first piece to rotate 4 times
        ...
        n = 0                                                           # resets the list item index
        ...
        for space in layout:
            ...
            if len(space) == 0:                                         # works on every space except the first one (already has been set)
                ...
                for _ in range(4):
                    ...
                    if compatibility_check(layout, layout[n], n) and n != 0: # checks if the piece matches it's surroundings
                        # layout[n] = new_pieces[n].copy()
                        if layout[1] != []:
                            # print(len(layout))
